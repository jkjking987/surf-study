...
function renderFavList(){
  ...
  const issueUrl = { bali: "bali.html", hainan: "hainan.html", board: "board.html" };
...
async function loadGlobalIndex(){
  ...
  const [bali, hainan, board] = await Promise.all([
    fetch("data/bali.json").then(r=>r.json()).catch(()=>({})),
    fetch("data/hainan.json").then(r=>r.json()).catch(()=>({})),
    fetch("data/board.json").then(r=>r.json()).catch(()=>({}))
  ]);
  const idx = [];
...
function doGlobalSearch(q){
  ...
  const issueLabels = { bali: ["BALI","tag-verm","bali.html"], hainan: ["HAINAN","tag-ocean","hainan.html"], board: ["BOARD","tag-must","board.html"] };
